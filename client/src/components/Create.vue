<template>
    ...
    <v-dialog v-model="dialog" width='500' color="blue-grey darken-4">
        ...
        <v-btn type="button" v-if="type == 'task'" color='teal accent-3' outlined v-on="on" id="dialog">
            Create Task
        </v-btn>
        <v-btn type="button" v-else-if="type == 'project'" color="teal accent-3" outlined v-on="on" id="dialog">
            Create Project
        </v-btn>
        </template>
